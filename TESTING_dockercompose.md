...
docker run --rm --net opin-demo_default jwilder/dockerize dockerize -wait tcp://mariadb:3306 -timeout 1m
```
...
docker ps --filter label=com.docker.compose.project=opin-demo | grep Up | grep opin-demo_nginx_1
docker ps --filter label=com.docker.compose.project=opin-demo | grep Up | grep opin-demo_mariadb_1
docker ps --filter label=com.docker.compose.project=opin-demo | grep Up | grep opin-demo_php_1
docker ps --filter label=com.docker.compose.project=opin-demo | grep Up | grep opin-demo_cli_1

# Should ssh against the cli container by default
docker-compose exec -T cli bash -c "env | grep LAGOON=" | grep cli-drupal

# Should have the correct environment set
docker-compose exec -T cli bash -c "env" | grep LAGOON_ROUTE | grep opin-demo.docker.amazee.io
